...
open class DTPagerController: UIViewController, UIScrollViewDelegate {
    ...
    private func setUpViewControllers() {
        ...
        for (index, viewController) in viewControllers.enumerated() {
            ...
            guard let _ = indexes.index(of: index) else {
                continue
            }

            // Add to call viewDidLoad if needed
            let height = view.bounds.height - segmentedControlHeight
            let x = CGFloat(index) * view.bounds.width
            viewController.view.frame = CGRect(x: x, y: 0, width: view.bounds.width, height: height)
